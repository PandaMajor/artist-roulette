...
export default function Home() {
  ...
  // const basic = Buffer.from(`${SPOTIFY_CLIENT_ID}:${SPOTIFY_SECRET}`).toString('base64')
  ...
  const getArtist = async (e: { preventDefault: () => void }) => {
    ...
    // Get the top tracks
    data = await axios.get('https://api.spotify.com/v1/artists/' + artists[randArtist] + '/top-tracks?market=US', {
      headers: {
        Authorization: `Bearer ${token}`
      },
      params: {
